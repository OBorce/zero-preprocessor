...
static struct access_modifier_ : x3::symbols<ast::access_modifier> {
  access_modifier_() {
    add("public", ast::access_modifier::PUBLIC)("protected",
...
static struct type_qualifier_ : x3::symbols<ast::TypeQualifier> {
  type_qualifier_() {
    add("inline", ast::TypeQualifier::Inline)("static", ast::TypeQualifier::Static)(
        "constexpr", ast::TypeQualifier::Constexpr)(
  ...
  binary_operator_() {
    add("+=", ast::Operator::PlusEq)("+", ast::Operator::Plus)(
        ...
        "<=", ast::Operator::LtEq)("<", ast::Operator::Lt)("&&",
                                                           ast::Operator::And)(
        "&=", ast::Operator::BitAndEq)("&", ast::Operator::BitAnd)(
        "||", ast::Operator::Or)("|=", ast::Operator::BitOrEq)(
        "|", ast::Operator::BitOr)("~=", ast::Operator::TildeEq)(
        "~", ast::Operator::Tilde)("^=", ast::Operator::BitXorEq)(
        "^", ast::Operator::BitXor)("!=", ast::Operator::NotEq)(
        "!", ast::Operator::Not)("==", ast::Operator::EqEq)("=", ast::Operator::Eq);
  }
} binary_operator;
...
auto const class_access_modifier_def = access_modifier >> optionaly_space >> ':';
...
auto const scope_end_def = optionaly_space >> '}' >> optionaly_space >> -lit(';');
...
auto const integral_def =
    ...
    (((lit("LLU") | "LLu" | "llU" | "llu") >>
     ...
     ((lit("LL") | "ll") >> x3::attr(VecUnqualifiedType{{{"long long int"}, {}}})) |
...
auto const floating_def =
    x3::omit[((digits >> '.' >> -digits) | (lit('.') >> digits))] >>
    x3::attr(ast::Literal{3.4}) >>
...
auto const char_literal_def =
    x3::omit[lit('\'') >> (char_ - '\'') >> '\''] >>
    x3::attr(ast::Literal{'x'}) >> x3::attr(VecUnqualifiedType{{{"char"}, {}}});

x3::rule<class pack_expension> const pack_expension = "char_literal";
auto const pack_expension_def = optionaly_space >> x3::lit("...");

x3::rule<class type_, ast::Type_> const type_ = "type_";
auto const type__def = name >>
                       *(lit("::") >> optionaly_space >>
...
x3::rule<class var_type_part, ast::unqulified_type> const var_type_part =
    "var_type_part";
...
auto const template_values_def =
    '<' >> optionaly_space >>
    (((type >> -(pack_expension)) | integral | string_literal) % arg_separator) >>
    '>';
...
auto const function_call_def = type >> optionaly_space >> '(' >> optionaly_space >>
                               optionaly_arguments >> optionaly_space >> ')';
// TODO: this allows prefix ++ on rvalues
auto const expression_old_def = (-(prefix_operator >> optionaly_space) >>
...
x3::rule<class optional_param, ast::var> const optional_param = "optional_param";
...
x3::rule<class optionaly_params, ast::params> const optionaly_params =
    "optionaly_params";
auto const optionaly_params_def = -(param_optionaly_default_old % arg_separator);
...
x3::rule<class parenthesis_begin, ast::RoundExpression> const parenthesis_begin =
    "parenthesis_begin";
...
x3::rule<class curly_begin, ast::CurlyExpression> const curly_begin = "curly_begin";
...
auto const lambda_capture_def = lit('[') >> optionaly_space >>
                                (']' |
                                 (((capture_default >> optionaly_space >>
                                    -(',' >> optionaly_space >> capture_params)) |
                                   capture_params) >>
                                  optionaly_space >> ']'));

x3::rule<class lambda_specifiers> const lambda_specifiers = "lambda_specifiers";
auto const lambda_specifiers_def = optionaly_space >> lit("mutable") | "constexpr";
...
x3::rule<class variable_expression, ast::UnqulifiedType> const variable_expression =
    "variable_expression";
...
x3::rule<class expression,
         std::variant<ast::VariableExpression,
...
x3::rule<class param_optionaly_default, ast::var> const param_optionaly_default =
    "param_optionaly_default";
...
// TODO: why is the ; added?
auto const var_with_init_def = param >> optionaly_space >> &(lit('=') | '{' | ';');
...
x3::rule<class if_statement, ast::IfStatement> const if_statement = "if_statement";
auto const if_statement_def = lit("if") >> optionaly_space >>
                              -lit("constexpr") >> optionaly_space
                              >> x3::attr(ast::IfStatement{});

x3::rule<class else_statement> const else_statement = "else_statement";
auto const else_statement_def = lit("else");

x3::rule<class else_if_statement, ast::IfStatement> const else_if_statement =
    "else_if_statement";
auto const else_if_statement_def = else_statement >> some_space >> if_statement;
...
auto const template_parameter_def = type_ >> ((pack_expension >> x3::attr(true)) |
                                              x3::attr(false)) >>
                                    -(some_space >> name) >>
...
auto const
    function_signature_old_def = -(template_parameters >> optionaly_space) >>
                                 bool_attr(lit("constexpr") >> some_space) >> type
                                 >> some_space >> name >> optionaly_space >>
                                 '(' >> optionaly_space >> optionaly_params >>
                                 ')' >> is_noexcept;

x3::rule<class user_class_template_deduction_guide, ast::UserDeductionGuide> const
    user_class_template_deduction_guide = "user_class_template_deduction_guide";
...
x3::rule<class function_start, ast::function_signature_old> const function_start =
    "function_start";
...
auto const
    function_declaration_def = -(template_parameters >> optionaly_space) >>
                               bool_attr(lit("constexpr") >> some_space) >> type
                               >> some_space >> name >> optionaly_space >> '(';
...
auto const operator_signature_def = -(template_parameters >> optionaly_space) >>
                                    ...
                                    >> all_overloadable_operators >> optionaly_space
                                    >> '(' >> optionaly_space >> optionaly_params >>
...
x3::rule<class class_inheritance, ast::class_inheritance> const class_inheritance =
    "class_inheritance";
...
auto const enumeration_def = lit("enum") >>
                             -(optionaly_space >> enum_class) >> some_space >> name
                             >>
                             -(optionaly_space >> ':' >> optionaly_space >> type_);
...
BOOST_SPIRIT_DEFINE(while_loop,
                    if_statement,
                    else_statement,
                    else_if_statement,
